...
async function main() {
    ...
    const folderId = '1jDV40IPmBhymPDcpmBOVByroj4THOy1e';

    const filesUrl = `https://www.googleapis.com/drive/v3/files?q='${folderId}' in parents and trashed=false&fields=files(id,name,modifiedTime)&${params}`;
    const filesRes = await fetch(filesUrl, { headers: { 'Authorization': `Bearer ${driveToken}` } });
    const filesData = await filesRes.json();

    let out = `FOLDER_ID: ${folderId}\n`;
    if (filesData.files) {
    ...
    fs.writeFileSync('folder_files_raw.txt', out);
    console.log('DONE');
}
