...
async function main() {
    try {
        ...
        const response = await fetch(`https://www.googleapis.com/drive/v3/files?pageSize=50&fields=files(id,name,modifiedTime)&orderBy=modifiedTime desc`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await response.json();
        fs.writeFileSync('drive_files.json', JSON.stringify(data.files, null, 2));
        console.log('Saved 50 recent files to drive_files.json');
    } catch (e) {
        console.error(e);
    }
}
