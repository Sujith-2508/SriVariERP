...
async function main() {
    try {
        ...
        const response = await fetch(`https://www.googleapis.com/drive/v3/files?pageSize=100&fields=files(id,name,mimeType,modifiedTime,shared)&orderBy=modifiedTime desc`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await response.json();
        console.log('DRIVE_FILES_START');
        data.files.forEach(f => {
            console.log(`${f.name} | ${f.id} | ${f.mimeType} | ${f.shared ? 'SHARED' : 'OWNED'}`);
        });
        console.log('DRIVE_FILES_END');
    } catch (e) {
        console.error(e);
    }
}
