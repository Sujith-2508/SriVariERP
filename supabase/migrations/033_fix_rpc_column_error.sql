-- Migration: Fix RPC column error for bill_payments sync
-- Reverts from UPDATE to INSERT for bill_payments to match Ledger Entry style.
-- Uses correct column names (bill_number instead of invoice_id).
...
BEGIN
    ...
    FOR v_invoice IN (
        ...
        -- A. Create allocation record (Core Logic)
        ...
        -- This is a history/ledger style insert used by the Desktop ERP's internal tracking.
        -- We wrapped it in a safe block to ensure mobile payments SUCCEED even if this side-table schema is messy.
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Log the error to Postgres log but don't fail the transaction
